...
const Process: React.FC = () => {
  return (
    <div className="pt-24 md:pt-32 pb-16 md:pb-24 min-h-screen bg-[#F9F6EE]">
      <div className="max-w-7xl mx-auto px-6">
        <header className="mb-12 md:mb-20 text-center max-w-3xl mx-auto">
          <span className="text-[#D4AF37] font-bold tracking-widest uppercase text-[10px] md:text-sm mb-4 block">The Journey</span>
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-serif font-bold mb-6 md:mb-8">Our Signature <span className="italic gold-text-gradient">Workflow</span></h1>
        ...
        <div className="relative">
          ...
          <div className="space-y-12 md:space-y-0 relative">
            {STEPS.map((step, index) => (
              <div key={index} className={`flex flex-col md:flex-row items-center ${index % 2 === 0 ? 'md:flex-row-reverse' : ''} mb-12 md:mb-20`}>
                <div className="w-full md:w-1/2 p-4 md:p-8 text-center">
                   <div className={`flex flex-col ${index % 2 === 0 ? 'md:items-end md:text-right' : 'md:items-start md:text-left'} items-center`}>
                      <span className="text-4xl md:text-6xl font-serif font-black text-[#D4AF37]/20 mb-2">{step.icon}</span>
                      <h3 className="text-xl md:text-2xl font-serif font-bold mb-4 text-neutral-900">{step.title}</h3>
                      <p className="text-sm md:text-base text-neutral-600 max-w-md leading-relaxed">{step.description}</p>
                   </div>
                </div>
                
                <div className="relative z-10 flex-shrink-0 my-6 md:my-0">
                  <div className="w-12 h-12 md:w-16 md:h-16 gold-gradient rounded-full flex items-center justify-center border-4 border-white shadow-xl">
                    <div className="w-2 h-2 bg-white rounded-full"></div>
                  </div>
                </div>
                ...
                <div className="md:w-1/2 p-8 hidden md:block">
                  <div className="w-full h-48 bg-white/50 border border-[#D4AF37]/20 rounded-2xl flex items-center justify-center">
                     <span className="text-[10px] tracking-widest uppercase text-neutral-400">Step {step.icon} Module</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="mt-12 md:mt-20 p-8 md:p-12 bg-white rounded-2xl md:rounded-3xl text-center border-2 border-[#D4AF37]/20">
          <h3 className="text-2xl md:text-3xl font-serif font-bold mb-4">A Stress-Free Experience</h3>
          <p className="text-sm md:text-base text-neutral-600 max-w-2xl mx-auto mb-8">
            Our project managers act as your single point of contact, ensuring that every milestone is met with 
            clarity and that your feedback is integrated seamlessly.
          </p>
          <div className="flex flex-wrap justify-center gap-4 md:space-x-8 opacity-50 grayscale text-sm md:text-xl italic font-serif">
