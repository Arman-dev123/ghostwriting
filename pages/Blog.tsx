...
const Blog: React.FC = () => {
  return (
    <div className="pt-24 md:pt-32 pb-16 md:pb-24 bg-white dark:bg-black min-h-screen transition-colors duration-500">
      <SEO 
        title="Ghostwriting Insights & Amazon Publishing Blog | Gold Leaf" 
        description="Expert guides on professional ghostwriting costs, Amazon KDP strategies, and author website branding from the leaders at Gold Leaf."
        url="https://goldleaf-ghostwriting-one.vercel.app/blog"
      />
      <div className="max-w-7xl mx-auto px-6">
        <header className="mb-10 md:mb-16 text-center md:text-left">
          <h1 className="text-3xl md:text-5xl font-serif font-bold mb-4 text-neutral-900 dark:text-white">
            The Gold Leaf <span className="italic gold-text-gradient">Review</span>
          </h1>
          <p className="text-neutral-500 dark:text-neutral-400 text-base md:text-lg">
            Strategic advice on **Amazon book publishing**, manuscript development, and **author branding**.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
          {BLOG_POSTS.map((post) => (
            <Link 
              key={post.id} 
              to={`${Page.Blog}/${post.slug}`} 
              className="group flex flex-col h-full bg-white dark:bg-neutral-900/30 border border-neutral-100 dark:border-neutral-800 rounded-2xl md:rounded-3xl overflow-hidden hover:shadow-2xl transition-all"
            >
              <div className="h-48 md:h-64 bg-neutral-100 dark:bg-neutral-800 overflow-hidden">
                <img 
                  src={`https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=800&q=80`} 
                  alt={`${post.title} - Professional Ghostwriting Advice`} 
                  className="w-full h-full object-cover grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700" 
                />
              </div>
              <div className="p-6 md:p-8 flex-grow flex flex-col">
                ...
                <h2 className="text-xl md:text-2xl font-serif font-bold mb-4 text-neutral-900 dark:text-white group-hover:text-[#D4AF37] transition-colors line-clamp-2">{post.title}</h2>
                <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-6 line-clamp-3">{post.excerpt}</p>
                <div className="mt-auto flex items-center text-[10px] font-bold uppercase tracking-widest text-neutral-900 dark:text-[#D4AF37]">
                  Read Guide
                  <svg className="w-3 h-3 ml-2 transform group-hover:translate-x-2 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                </div>
              </div>
